...
async function updateRoomStatuses() {
    try {
        const res = await fetch('/instructor/instructor_classes.php'); // Endpoint to get JSON data
        ...
        data.forEach(building => {
            ...
            if (b) {
                ...
                // Update individual room statuses
                if (b.rooms) {
                    b.rooms.forEach(room => {
                        ...
                        // Check class schedules to set status automatically
                        const roomClasses = classesByRoom[room.room_id] || [];
                        let isOccupied = false;

                        for (const c of roomClasses) {
                            const [startH, startM] = c.schedule_start.split(':').map(Number);
                            const [endH, endM] = c.schedule_end.split(':').map(Number);
                        ...
                        if (!isOccupied) room.status = 'available';
                    });
                }
            }
        });

        // Re-render rooms for selected building
