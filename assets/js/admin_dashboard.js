...
document.querySelectorAll('[data-action="select-building"]').forEach(item => {
    item.addEventListener('click', () => {
        ...
        // Highlight selected building (optional)
    ...
    try {
        ...
        data.forEach(building => {
            ...
            if (b) {
                ...
                // Also update individual room statuses if available
                if (b.rooms) {
        ...
        // If a building is selected, re-render its rooms to show updated statuses
        if (window.currentBuildingId) renderRooms(window.currentBuildingId);

        // Update the small status counts in the building list
        data.forEach(building => {
